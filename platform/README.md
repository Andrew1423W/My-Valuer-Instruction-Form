...
| **Report assembly** | One-click `.docx` in My Valuer styling: instruction and property tables, property description, accommodation schedule, comparable summary table with ratings, per-sale commentary, rating tally, valuation conclusions |
...
- **docx** for Word report generation
...
src/
  app/
    ...
    api/jobs/[id]/report/        .docx generation
  ...
  lib/report.ts                  Word report assembly
  lib/status.ts                  job stages
  lib/session.ts                 signed-in valuer
```

The data model is the thing to read first: `src/db/schema.ts`.
